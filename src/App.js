...
import "./App.css";
import initialaizeAuthentication from "./Firebase/firebase.init";
...
function App() {
  ...
  const handleSignUp= (e)=>{
    ...
    if(!/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/.test(userPassword)){
      setError("doest fulfilled")
      return;
